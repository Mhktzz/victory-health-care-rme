...
<div class="bg-white shadow rounded-xl">
    <form action="{{ route('dashboard.superadmin.datapasien.update', $patient) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="telepon" value="{{ $patient->no_hp }}"
                    class="w-full border rounded-lg px-4 py-2">
            </div>
        </div>

        <div class="flex justify-end px-6 py-4 border-t">
            <a href="{{ route('dashboard.superadmin.datapasien.index') }}" class="px-4 py-2 bg-gray-200 rounded-lg">Batal</a>
            <button class="px-6 py-2 bg-orange-600 text-white rounded-lg">Update</button>
        </div>
    </form>
</div>
